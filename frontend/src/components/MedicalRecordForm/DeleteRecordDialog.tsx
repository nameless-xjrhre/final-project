...
const DeleteRecord = gql`
  mutation DeleteMedicalRecord($id: Int!) {
...
export default function DeleteRecordDialog({
  ...
  const [, deleteRecord] = useMutation(DeleteRecord)
    ...
    deleteRecord(medicalRecordId)
      .then((result) => {
        if (result.error) {
          ...
          handleClose(handleComplete)
          showSuccessAlert('Successfully deleted medical record.')
        }
      })
